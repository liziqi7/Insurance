...
(function(win) {
    ...
    */
     ...
     * @param  {[Boolen]} isHideLoad   [是否显示加载进度条,默认显示加载进度条]
     */
    Jser.getJSON = function(url, data, sfn, errfn, method, datatype, isHideLoad) {
        var t = this;
        data = data || "";
        !isHideLoad && $("#js-loading").show();
        // 关闭全局AJAX缓存
        // $.ajaxSetup({
        //  cache: false 
        // });
            ...
            $.ajax({
                ...
                error: function(e, xhr, opt) {
                    ...
                    if (xhr == "abort") {
                        ...
                        e.data = data;
                        Jser.log("------------------------error↓--------------------------");
                        Jser.log("error:url:" + url + ",data：" + data + ",sfn" + sfn + ",errfn" + errfn + ",method" + method + ",datatype" + datatype + ",isload" + isload + "状态: " + e.status + " " + e.statusText);
                        Jser.log("错误行为分析：")
                        Jser.log("说明你的请求并没有返回值,或者返回值超时")
                        Jser.log("1.检测是否断网了")
                        Jser.log("2.检测当前参数" + data + "是否正确")
        ...
        Jser.getJSON(url, {
            ...
            if (window.wx) {
                ...
                wx.error(function(res) {
                    ...
                    Jser.getJSON(url + "/update_access_token/", '', function(data) {
                        Jser.WXConfig();
                    })
                });
            }
    ...
    Jser.setShare = function(params) {
        if ($.isPlainObject(params) && !$.isEmptyObject(params) && window.WeiXinShare) {
            $.extend(params, WeiXinShare);
        }
    };
    // 显示分享
    Jser.share = function(params) {
        if (params) {
            Jser.setShare(params);
        }
        Jser._showShare();
    };
    Jser._showShare = function() {
        var $elem = $("#js-weixin-share");
        $elem.fadeIn(300, function() {
            $elem.one("touchstart.share", Jser._hideShare);
        });
    };
    Jser._hideShare = function() {
     ...
     */
    Jser.switchScene = function(s1, s2, sc1, sc2) {
    ...
    Jser._showLoad = function() {
        var $elem = $("#js-loading");
        $elem.fadeIn(300, function() {
            $elem.one("touchstart.loading", Jser._hideLoad);
        });
    };
    Jser._hideLoad = function() {
        $("#js-loading").fadeOut(300);
    };
    Jser.nextFrame = (function() {
